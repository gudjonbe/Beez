...
# ---------------------------------------------------------------------------
# Minimal CSV logger for runs/<run_id>/frames.csv
# ---------------------------------------------------------------------------
...
class _CSVLogger:
    ...
    def _flatten(self, frame: Dict[str, Any]) -> Dict[str, Any]:
        out: Dict[str, Any] = {}
    ...
    def write(self, frame: Dict[str, Any]) -> None:
        ...
        if self._fieldnames is None:
            # fixed header from first frame (keeps things simple/stable)
            self._fieldnames = sorted(flat.keys())
            self._f = self.csv_path.open("w", newline="", encoding="utf-8")
            self._writer = csv.DictWriter(self._f, fieldnames=self._fieldnames)
            self._writer.writeheader()
        if self._writer is None:
            return
        row = {k: flat.get(k, None) for k in self._fieldnames}
        self._writer.writerow(row)
        # flush lightly so file shows up during run
        try:
            self._f.flush()  # type: ignore[union-attr]
        except Exception:
            pass

_logger = _CSVLogger()  # created at import time; safe and simple
...
async def sim_loop() -> None:
    ...
    print(f"[sim] loop starting @ {SIM_TICK_HZ} Hz  | run_id={_logger.run_id}  -> {(_logger.csv_path)}")
    ...
    while True:
        ...
        try:
            _sim.step(dt)
            # log every tick
            _logger.write(_sim.get_view())
        except Exception as e:
            # Never let a transient sim error kill the loop.
            print("[sim] step/log error:", repr(e))
            traceback.print_exc()
...
# --------------------- added: metrics page + WS (read-only) ---------------------

async def metrics_page(request):
    """Serve the plotting tab."""
    return FileResponse(
        UI_DIR / "metrics.html",
        headers={
            "Cache-Control": "no-store, max-age=0",
...
async def ws_metrics_endpoint(ws: WebSocket):
    """Stream raw frames for plotting; read-only, separate from /ws."""
    await ws.accept()
    hz = 20  # plotting rate; independent of SIM_TICK_HZ
    try:
        while True:
            await ws.send_text(json.dumps(_sim.get_view()))
            await asyncio.sleep(1.0 / hz)
    except WebSocketDisconnect:
        pass

routes = [
    Route("/metrics", endpoint=metrics_page),  # added
    Route("/", endpoint=homepage),
    WebSocketRoute("/ws", endpoint=ws_endpoint),
    WebSocketRoute("/ws/metrics", endpoint=ws_metrics_endpoint),  # added
...
async def _on_start() -> None:
    ...
    if _loop_task is None:
        ...
        print(f"[app] sim loop task created | logging to runs/{_logger.run_id}/frames.csv")
