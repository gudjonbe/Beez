...
class WorkerBee(Bee):
    ...
    Trip:
        wander → (maybe adopt advert) → to_flower → collect → chain → to_hive
        → deposit (+advertise if profitable) → wander

    Notes:
    - Recruitment is modeled via a simple "dance floor" (world.comms).
    - Deposits enqueue into the hive intake queue (world.deposit(amount)).
    - Advertising strength is gently throttled if the intake queue is saturated
      (a simple stand-in for tremble-dance dynamics).
    """
    ...
    def _go_towards(self, x: float, y: float, dt: float, speed_scale: float = 0.6) -> None:
        """Turn toward (x,y) and step position."""
    ...
    def step(self, dt: float, width: int, height: int, rng, world: Any | None = None) -> None:
        if world is None:
            # Fallback motion (random-walk + bounce) if world isn’t provided.
        ...
        if self.state == "wander":
            ...
            if (self._recruit_timer <= 0.0 and
                ...
                if hasattr(world, "comms"):
                    target = world.comms.sample()
        ...
        # ------------------------- TO_FLOWER --------------------------------
            ...
            # Give up if we’ve been seeking too long (stuck).
            self._seek_timer += dt
            ...
            if dist < self.ARRIVAL_RADIUS:
                ...
                # If nothing was collected, resume searching (don’t go home empty).
                if got <= 1e-6:
                    self.target_flower_id = None
                    self.state = "wander"
                    return
        ...
        # ------------------------- TO_HIVE ----------------------------------
            ...
            if dist < (world.hive_radius + 10.0):
                # Enqueue nectar into the hive intake queue.
                world.deposit(self.carry)

                # Optionally throttle recruitment if the intake is congested.
                # (Simple proxy for tremble-dance: weaker/shorter adverts when backlog is high.)
                throttle = 1.0
                if hasattr(world, "intake_queue") and hasattr(world, "intake_rate"):
                    horizon = max(1e-6, world.intake_rate * 3.0)   # ~3 sec capacity
                    load = world.intake_queue / horizon           # 0 = empty, >1 congested
                    throttle = 1.0 / (1.0 + 3.0 * max(0.0, load)) # 1..0 as load grows

                # Waggle-like advert if last sip was worthwhile.
                if self._last_flower_xy and self._last_take >= self.ADVERTISE_MIN and hasattr(world, "comms"):
                    base_strength = max(0.2, min(2.0, self._last_take / max(1e-6, self.VISIT_QUANTA)))
                    strength = base_strength * throttle
                    ttl = self.ADVERTISE_TTL * (0.5 + 0.5 * throttle)
                    world.comms.advertise(self._last_flower_xy[0], self._last_flower_xy[1],
                                          strength=strength, ttl=ttl)

                # Reset trip memory and head out again.
                self.carry = 0.0
